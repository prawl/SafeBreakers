...
namespace TextFx
{
	public class TextEffectsManager : EditorWindow
	{
		...
		const float INACTIVE_EXIT_PAUSE_DURATION = 0.000001f;
...
//					m_save_button_texture,
	    ...
	    static void Init ()
		{
			...
			EditorWindow window = EditorWindow.GetWindow (typeof (TextEffectsManager));

			window.title = "TextFx Editor";
	    }
...
//		}
		...
		void OnEnable()
	    {
			...
			m_copy_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/CopyButton_pro.png", typeof(Texture2D)) as Texture2D;
			m_paste_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/PasteButton_pro.png", typeof(Texture2D)) as Texture2D;
//			m_save_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/SaveButton_pro.png", typeof(Texture2D)) as Texture2D;
			m_play_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/PlayButton_pro.png", typeof(Texture2D)) as Texture2D;
			m_pause_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/PauseButton_pro.png", typeof(Texture2D)) as Texture2D;
			m_reset_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/ResetButton_pro.png", typeof(Texture2D)) as Texture2D;
			m_continue_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/ContinueButton_pro.png", typeof(Texture2D)) as Texture2D;
			m_continue_past_loop_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/ContinueFromLoop_pro.png", typeof(Texture2D)) as Texture2D;
			m_repeat_off_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/RepeatButton2.png", typeof(Texture2D)) as Texture2D;
			m_repeat_on_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/RepeatButton_pro2.png", typeof(Texture2D)) as Texture2D;
			m_infinity_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/InfinitySymbol_pro.png", typeof(Texture2D)) as Texture2D;
#else

			m_copy_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/CopyButton.png", typeof(Texture2D)) as Texture2D;
			m_paste_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/PasteButton.png", typeof(Texture2D)) as Texture2D;
//			m_save_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/SaveButton.png", typeof(Texture2D)) as Texture2D;
			m_play_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/PlayButton.png", typeof(Texture2D)) as Texture2D;
			m_pause_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/PauseButton.png", typeof(Texture2D)) as Texture2D;
			m_reset_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/ResetButton.png", typeof(Texture2D)) as Texture2D;
			m_continue_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/ContinueButton.png", typeof(Texture2D)) as Texture2D;
			m_continue_past_loop_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/ContinueFromLoop.png", typeof(Texture2D)) as Texture2D;
			m_repeat_off_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/RepeatButton_pro2.png", typeof(Texture2D)) as Texture2D;
			m_repeat_on_button_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/RepeatButton2.png", typeof(Texture2D)) as Texture2D;
			m_infinity_texture = AssetDatabase.LoadAssetAtPath("Assets/TextFx/Editor/Icons/InfinitySymbol.png", typeof(Texture2D)) as Texture2D;
#endif
	    }
		...
		{	
			...
			{
				if(m_animation_manager == null || m_textfx_animation_interface == null)
				{
		...
		void LoadInPresetAnimationNames()
		{
			// Load in the names and categories of all the animation configurations available from the anim config file
			TextAsset config_titles_file = AssetDatabase.LoadAssetAtPath("Assets/TextFx/AnimationConfigs/AnimationConfigs2.txt", typeof(TextAsset)) as TextAsset;
			
			string[] config_file_lines = config_titles_file.text.Split(new char[]{'\n'});
			
			m_animation_config_titles = new string[config_file_lines.Length];
			
			for(int idx=0; idx < config_file_lines.Length; idx++)
			{
				m_animation_config_titles[idx] = config_file_lines[idx];
			}

			LoadInAnimationPartNames ("AnimationConfigIntros", ref m_animation_config_intro_titles);
			LoadInAnimationPartNames ("AnimationConfigMains", ref m_animation_config_main_titles);
			LoadInAnimationPartNames ("AnimationConfigOutros", ref m_animation_config_outro_titles);
		}

		void LoadInAnimationPartNames(string fileName, ref string[] configList)
		{

			// Load in the names and categories of all the animation configurations available from the anim config file
			TextAsset config_title_file = AssetDatabase.LoadAssetAtPath("Assets/TextFx/AnimationConfigs/" + fileName + ".txt", typeof(TextAsset)) as TextAsset;
			
			string[] file_lines = config_title_file.text.Split(new char[]{'\n'});

			configList = new string[file_lines.Length + 1];

			configList [0] = "None";
				
			for(int idx=0; idx < file_lines.Length; idx++)
			{
				configList[idx + 1] = file_lines[idx];
			}
...
//		}
	...
	//							m_animation_manager.PrepareAnimationData();
		...
		void PauseEditorAnimation()
		{
			m_paused = !m_paused;
			m_animation_manager.Paused = m_paused;
		}

		void PlayEditorAnimation(int starting_action = 0)
		{
			if (m_animation_manager.m_master_animations == null || m_animation_manager.m_master_animations.Count == 0)
			{
			...
			m_animation_manager.PlayAnimation(starting_action_index: starting_action);
			m_paused = false;
			m_animation_manager.Paused = false;
		}
...
//						Debug.Log("Importing from : " + path + ", animation_config_data : " + (animation_config_data != null));
		...
		void UpdatePresetAnimSectionActionIndexes()
		{
			int action_idx = 0;
			int loop_idx = 0;

			m_animation_manager.m_preset_intro.m_start_action = 0;
			if(m_animation_manager.m_preset_intro.m_active)
				action_idx += m_animation_manager.m_preset_intro.m_num_actions + 1;		// Plus one for the exit pause action

			m_animation_manager.m_preset_main.m_start_action = action_idx;
			if(m_animation_manager.m_preset_main.m_active)
				action_idx += m_animation_manager.m_preset_main.m_num_actions + 1;		// Plus one for the exit pause action

			m_animation_manager.m_preset_outro.m_start_action = action_idx;


			// Calculate Loop indexes
			m_animation_manager.m_preset_intro.m_start_loop = 0;
			loop_idx += m_animation_manager.m_preset_intro.m_num_loops;

			m_animation_manager.m_preset_main.m_start_loop = loop_idx;
			loop_idx += m_animation_manager.m_preset_main.m_num_loops;

			m_animation_manager.m_preset_outro.m_start_loop = loop_idx;
		}
...
//			}
			...
			{
				gui_y_offset += LINE_HEIGHT;
				m_animation_manager.m_begin_delay = EditorGUI.FloatField(new Rect(MainEditorX + 20, gui_y_offset, ENUM_SELECTOR_WIDTH, LINE_HEIGHT), new GUIContent("Delay", "How much of a delay should there be before automatically playing this animation?"), m_animation_manager.m_begin_delay);
			}
			...
			if(selected_animation.m_letters_to_animate_option == LETTERS_TO_ANIMATE.CUSTOM)
			{
				...
				CUSTOM_LETTERS_LIST_POS = GUI.BeginScrollView(new Rect(MainEditorX,gui_y_offset,main_editor_width,LINE_HEIGHT*3), CUSTOM_LETTERS_LIST_POS,  new Rect(0,0,m_textfx_animation_interface.Text.Length * 20,40));

				int idx = 0;
				float spacing_offset = 0;
				foreach(char character in m_textfx_animation_interface.Text)
				{
	...
	//					spacing_offset += 20;
			...
			CheckGUIChange(0, true, ANIMATION_DATA_TYPE.ANIMATE_ON);
					...
					if(action.AudioEffectsEditorDisplay)
					{
						...
						for(int idx=0; idx < action.NumAudioEffectSetups; idx++)
						{
							...
							if(audio_setup.m_editor_display)
							{
								...
								{
									audio_setup.CUSTOM_LETTERS_LIST_POS = GUI.BeginScrollView(new Rect(MainEditorX,gui_y_offset,main_editor_width,LINE_HEIGHT*3), audio_setup.CUSTOM_LETTERS_LIST_POS,  new Rect(0,0,m_textfx_animation_interface.Text.Length * 20,40));
						...
						for(int idx=0; idx < action.NumParticleEffectSetups; idx++)
						{
							...
							if(particle_setup.m_editor_display)
							{
								...
								if(particle_setup.m_effect_assignment == PLAY_ITEM_ASSIGNMENT.CUSTOM)
								{
									...
									particle_setup.CUSTOM_LETTERS_LIST_POS = GUI.BeginScrollView(new Rect(MainEditorX,gui_y_offset,main_editor_width,LINE_HEIGHT*3), particle_setup.CUSTOM_LETTERS_LIST_POS,  new Rect(0,0,m_textfx_animation_interface.Text.Length * 20,40));
