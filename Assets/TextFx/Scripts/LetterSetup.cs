...
namespace TextFx
{
	...
	public class LetterSetup
	{
		...
		public int LetterIdx { get { return m_progression_variables != null ? m_progression_variables.m_letter_value : -1; } }
		...
		public LetterSetup(TextFxAnimationManager animation_manager_ref)
		{
		...
		public void SetLetterData(Vector3[] mesh_verts, Color[] mesh_cols, int mesh_index)
		{
			m_base_vertices = mesh_verts;

			m_base_colours = mesh_cols != null && mesh_cols.Length == 4 ? new VertexColour(mesh_cols[0], mesh_cols[1], mesh_cols[2], mesh_cols[3]) : new VertexColour(Color.white);
		...
		public void SetMeshState(TextFxAnimationManager animation_manager_ref, int action_idx, float action_progress, LetterAnimation animation, AnimatePerOptions animate_per, ref Vector3[] mesh_verts, ref Color[] mesh_colours)
		{
			...
			else
			{
				...
				m_base_vertices.CopyTo(mesh_verts, 0);
			}
		}
...
//			Debug.Log("SetNextActionIndex() Num Active Loops : "+ m_anim_state_vars.m_active_loop_cycles.Count);
			...
			for(int idx=0; idx < animation.NumLoops; idx++)
			{
				...
				if(loop.m_start_action_idx == m_anim_state_vars.m_action_index)
				{
					...
					if(loop_idx >= 0)
					{
						...
						m_anim_state_vars.m_active_loop_cycles.Insert(loop_idx, loop);
					}
				}
			}
		}
...
//			Debug.Log ("CallContinueCallback " + m_mesh_index);
		...
		public void AnimateMesh(	TextFxAnimationManager animation_manager_ref,
		                                          	bool force_render,
													float timer,
...
//						Debug.Log("Waiting at the start of loop, action index : " + m_anim_state_vars.m_action_index);
...
//					if(m_mesh_index == 3)
//						Debug.Log("[" + m_mesh_index + "] Force letter to wait for rest of letters to be in sync");
...
//							Debug.Log("[" + m_mesh_index + "] current action index : "+  m_anim_state_vars.m_action_index + ", triggerIndex : "+ m_continueActionIndexTrigger + ", [" + ActiveLoopCycles[0].m_active_loop_index + "] loop_num_left : "+ ActiveLoopCycles[0].m_number_of_loops);
		...
		{
			if (m_stub_instance || current_action.m_action_type == ACTION_TYPE.BREAK)
			{
				return;
			}
		...
		void SetupMesh(LetterAction letter_action, float action_progress, float linear_progress, ref Vector3[] mesh_verts, ref Color[] mesh_colours)
		{
			...
			if(m_current_animated_vertices == null || m_current_animated_vertices.Length != 4)
				m_current_animated_vertices = new Vector3[4];
			...
			// Sort out letters colour
			mesh_colours = new Color[]{
										Color.Lerp(m_colour_from.top_right, m_colour_to.top_right, action_progress), 
										Color.Lerp(m_colour_from.top_left, m_colour_to.top_left, action_progress), 
										Color.Lerp(m_colour_from.bottom_left, m_colour_to.bottom_left, action_progress), 
										Color.Lerp(m_colour_from.bottom_right, m_colour_to.bottom_right, action_progress)};

			if(m_current_animated_colours == null || m_current_animated_colours.Length != 4)
				m_current_animated_colours = new Color[4];
			
			mesh_colours.CopyTo (m_current_animated_colours, 0);


			// Cache the current colour of this letter
			if(m_letter_colour == null)
				m_letter_colour = new VertexColour();
			
			m_letter_colour.top_right = mesh_colours [0];
			m_letter_colour.top_left = mesh_colours [1];
			m_letter_colour.bottom_left = mesh_colours [2];
			m_letter_colour.bottom_right = mesh_colours [3];
		}
