...
namespace TextFx
{
	...
	public abstract class ActionVariableProgression
	{
		...
		public abstract Boomlagoon.JSON.JSONValue ExportData();
...
//			}
		...
		public override Boomlagoon.JSON.JSONValue ExportData()
		{
			Boomlagoon.JSON.JSONObject json_data = base.ExportData().Obj;
		...
		public float DrawPositionEditorGUI(GUIContent label, Rect position, bool offset_legal, bool unique_random_legal = false, bool bold_label = true)
		{
			...
			if(Progression != (int) ValueProgression.Eased && Progression != (int) ValueProgression.EasedCustom)
			{
	...
	public class ActionVector3Progression : ActionVariableProgression
	{
		...
		Vector3 GetValue(List<LetterAction> all_letter_actions, int progression_idx)
		{
			...
			{
				vecValue = m_offset_progression.GetVector3Prog(all_letter_actions).GetValue(all_letter_actions, progression_idx);
			}
